...
%%%%%%%%%%%%%%%%%%%%
 ...
 VAR.select_vars      = {'APITR','ACITR','GOV','RGDP','DEBT','EMP','HperW','LF'};
 VAR.vars             = DATASET.TSERIES(:,cell2mat(values(DATASET.MAP,VAR.select_vars)));
 VAR.MAP              = containers.Map([VAR.select_vars,{'UNR'}],[1:size(VAR.vars,2) 9]);
 ...
 % Figure 9: APITR Cut
 %%%%%%%%%%%%%%%%%%%%%
 FIG.axes(:,cell2mat(values(VARO1.MAP,{'EMP'})))    = [-1.25;2];
 FIG.axes(:,cell2mat(values(VARO1.MAP,{'HperW'})))  = [-1;1];
 FIG.axes(:,cell2mat(values(VARO1.MAP,{'LF'})))    = [-1;1];
 FIG.axes(:,cell2mat(values(VARO1.MAP,{'UNR'})))    = [-1.5;1];
 plotdisplay = {'EMP','HperW','LF','UNR'};
