%==========================================================================
% This script produces Figure 3 of "Proxy SVARs: Asymptotic Theory, 
...
VARData = [VARDataAll(:,1:2),VARDataAll(:,5:7),VARDataAll(:,11:13)];
...
for order = 1:2
    ...
    IRF(N+1,:,order) = (1 - 0.9475)*(exp(-0.9475/(1 - 0.9475)*...
        (IRF(6,:,order) - IRF(8,:,order))/100) - 1)*100;
end
...
for boot = 1:nBoot
    if toggleBoot == 1
        ...
        %center the bootstrapped errors and proxy variables
    ...
    for order = 1:2
        ...
        for j = 2:nImp
            ...
            IRF_boot(N+1,:,boot,order) = (1 - 0.9475)*...
                (exp(-0.9475/(1 - 0.9475)*...
                (IRF_boot(6,:,boot,order) - IRF_boot(8,:,boot,order))/100) - 1)*100;
        end
    end
end

%sort the bootstrapped IRFs
IRF_bootSort = zeros(N+1,nImp,nBoot,2);
for order = 1:2
    for n = 1:N+1
        for j = 1:nImp
...
IRF_confidence68 = zeros(N+1,nImp,2,2);
...
IRF_confidence90 = zeros(N+1,nImp,2,2);
...
hold on
plot(IRF(6,:,1),'b-')
plot(IRF_confidence68(6,:,1,1),'b--')
plot(IRF_confidence68(6,:,2,1),'b--')
plot(IRF_confidence90(6,:,1,1),'b-.')
plot(IRF_confidence90(6,:,2,1),'b-.')
hold off
xlim([1,nImp])
xlabel('Quarters')
ylabel('Percent')
...
hold on
plot(IRF(6,:,2),'b-')
plot(IRF_confidence68(6,:,1,2),'b--')
plot(IRF_confidence68(6,:,2,2),'b--')
plot(IRF_confidence90(6,:,1,2),'b-.')
plot(IRF_confidence90(6,:,2,2),'b-.')
hold off
xlim([1,nImp])
xlabel('Quarters')
ylabel('Percent')
...
hold on
plot(IRF(9,:,1),'b-')
plot(IRF_confidence68(9,:,1,1),'b--')
plot(IRF_confidence68(9,:,2,1),'b--')
plot(IRF_confidence90(9,:,1,1),'b-.')
plot(IRF_confidence90(9,:,2,1),'b-.')
hold off
xlim([1,nImp])
xlabel('Quarters')
ylabel('Percent')
...
hold on
plot(IRF(9,:,2),'b-')
plot(IRF_confidence68(9,:,1,2),'b--')
plot(IRF_confidence68(9,:,2,2),'b--')
plot(IRF_confidence90(9,:,1,2),'b-.')
plot(IRF_confidence90(9,:,2,2),'b-.')
hold off
xlim([1,nImp])
xlabel('Quarters')
ylabel('Percent')
