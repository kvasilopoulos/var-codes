...
switch_extern       =   0;  % run VARs with particular variables added one-by-one (time consuming, Figures 3-6)
VAR.switch_exp      =   1;      % 0 or 1 to calculate expectation based VAR (Figure 6)

%Setting up possible specifications
smpl_min_VAR_vec    =   [1979 7]; %[year month;year month]    
smpl_max_VAR_vec    =   [2012 6]; %[2012 6; 2008 6]; %                                      %End-of-sample (change for robustness exercises)   
monpol_vars_cell     =   {'GS1'};    %{'FF','GS1','GS2'};                                   %Monetary policy variables (change for robustness exercises)  
monpol_vars_label_cell =  {'1 year rate'};  %{'Fed Funds Rate','1 year rate','2 year rate'};%Names of the monetary policy variables    
monpol_vars_label_short_cell=  {'1YR'};  %{'FF','1YR','2YR'};                               %Short names of the monetary policy variables    
